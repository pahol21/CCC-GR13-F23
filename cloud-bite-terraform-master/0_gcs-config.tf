...
resource "google_logging_project_sink" "top_level_sink" {
  ...
  filter         = "resource.type=\"cloud_run_revision\" OR resource.type=\"storage.googleapis.com/Bucket\" OR resource.type=\"your_database_resource_type\""
...
resource "google_project_iam_binding" "bigquery_data_editor_binding" {
  ...
  members = [
    "serviceAccount:${google_logging_project_sink.my_sink.writer_identity}"
  ]
}